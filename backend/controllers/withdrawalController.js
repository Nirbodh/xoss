...
exports.requestWithdrawal = async (req, res) => {
  ...
  try {
    ...
    // ✅ FIXED: Check user balance first (for better error message)
    const user = await User.findById(userId).session(session);
    if (!user || user.wallet_balance < parsedAmount) {
      await session.abortTransaction();
      session.endSession();
      return res.status(400).json({ 
        success: false, 
    ...
    // ✅ FIXED: Update Wallet AND User in same transaction
    ...
    // ✅ FIXED: Update User wallet_balance
    await User.findByIdAndUpdate(
      userId,
      { 
        $inc: { 
          wallet_balance: -parsedAmount,
          total_earnings: -parsedAmount 
        } 
      },
      { session }
    );
    ...
    const updatedUser = await User.findById(userId).select('username email phone wallet_balance');
...
exports.getUserWithdrawals = async (req, res) => {
  try {
    ...
    const withdrawals = await Withdrawal.find(filter)
      .populate('user_id', 'username email phone wallet_balance') // ✅ wallet_balance যোগ করা হয়েছে
      .sort({ createdAt: -1 })
      .limit(Number(limit))
...
exports.getWithdrawalStats = async (req, res) => {
  try {
    ...
    const wallet = await Wallet.findOne({ user_id: userId });
    const user = await User.findById(userId).select('wallet_balance total_earnings');

    return res.json({
      success: true,
      data: {
        userBalance: user?.wallet_balance || 0, // ✅ User balance
        walletBalance: wallet?.balance || 0,    // ✅ Wallet balance (should be same)
...
exports.getPendingWithdrawals = async (req, res) => {
  try {
    ...
    const withdrawals = await Withdrawal.find({ status: 'pending' })
      .populate('user_id', 'username email phone avatar wallet_balance') // ✅ wallet_balance যোগ করা হয়েছে
...
exports.rejectWithdrawal = async (req, res) => {
  ...
  try {
    ...
    // ✅ FIXED: Refund wallet AND user balance
    const wallet = await Wallet.findOneAndUpdate(
    ...
    // ✅ FIXED: Also refund user wallet_balance
    await User.findByIdAndUpdate(
      withdrawal.user_id,
      { 
        $inc: { 
          wallet_balance: withdrawal.amount,
          total_earnings: withdrawal.amount 
        } 
      },
      { session }
    );
